...
def fetch_user_history_rows(username, start_date, end_date, comment_keys, only_authored):
   ...
   # Iterate through the user history. Note that the comment operations returned from the history api call are not
   # the same as the full comments, they are missing certain information such as root author and root permlink.
   for comment_op in fetch_user_comment_ops(username):
      ...
      if op_date != current_date:
         ...
         print_with_timestamp('Searching history for {}'.format(current_date))

      post_comments = fetch_comment_op_thread(comment_op, comment_keys, username, only_authored)
      num_post_comments = len(post_comments)
      if num_post_comments > 0:
         num_posts += 1
         num_comments += num_post_comments
      yield format_comment_rows(post_comments)
   
   print_with_timestamp('Fetched {} posts with {} new comments'.format(num_posts, num_comments))
...
def fetch_user_comment_ops(username):
   return steem.account.Account(username, steemd).history_reverse(filter_by='comment', batch_size=3000)
