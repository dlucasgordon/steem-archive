...
def display_thread(author, permlink):
   ...
   if not result:
      print('Could not find thread \'{}\' in the database'.format(get_link_string(author, permlink)))
      return
   comment = result[0]

   root_author, root_permlink = get_root_key(comment)
   post_comments = list(db.select(db.comment, dict(root_author=root_author, root_permlink=root_permlink)))
...
def build_comment_tree(comment, parent_groups):
   return dict(
      comment=comment,
      children=[build_comment_tree(c, parent_groups) for c in parent_groups.get((comment['author'], comment['permlink']), [])],
   )
...
def print_comment(c, depth):
   indent = '    ' * depth
   header_message = '@{}: {}'.format(c['author'], str(c['created'])[:-3])
   if c['was_deleted']:
   ...
   if depth == 0:
      ...
      print(colored(get_header_string(get_link_string(*get_key(c))) + '\n\n', 'green', attrs=['bold']))
