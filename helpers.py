...
def timer(message):
   ...
   print_with_timestamp('{}'.format(message))
   try:
      yield
   finally:
      end_time = datetime.now()
      delta = end_time - start_time
      print_with_timestamp('{}: {}'.format(message, colored(get_timedelta_str(delta), 'cyan')))

def print_with_timestamp(message):
   datetime_str = colored(get_datetime_str(datetime.now()), 'cyan')
   print('[{}] {}'.format(datetime_str, message))

def get_datetime_str(time):
   return str(time)[:-4]

# Round a timedelta seconds to two digits and return a string representation
