...
def select(table, cols, conn=get_conn()):
   stmt = table.select()
...
# Stores the comments/posts. Note that the columns without default values are named the same as they appear in the steem api.
comment = Table(
   'comment', metadata,
